...
export class CProcedimentoPage implements OnInit {
  ...
  async beforeTakePicture() {
    ...
    {
      const alert = await this.alertCtrl.create({
        //message: 'Captura de Imagem',
        subHeader: 'Captura de Imagem',
        inputs: [{type: 'radio', label: 'Galeria', value: '1', checked: false},{type: 'radio', label: 'Câmera', value: '2', checked: true}],
        buttons: [{text: 'Cancelar'}, {text: 'OK', handler: data => {this.takePicture(data);}} ],
       });
       await alert.present(); 
    }
    else{
  ...
  uploadPicture(blob: Blob) {
    ...
    task.snapshotChanges().pipe( 
      finalize(() => this.downloadUrl2 = ref.getDownloadURL.toString())
    ).subscribe()

    console.log("lalala" + this.downloadUrl2)
